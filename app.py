...
def _classificar_aplicacoes(dados: pd.DataFrame) -> pd.DataFrame:
    ...
    validado manualmente (GRUPOS_VALIDADOS) quando disponível; caso
    contrário, cai para a heurística de identificar o grupo com mais
    cores/cadernos distintos como a aplicação Regular. Descarta todos
    os demais grupos (PPL, reaplicação, digital, adaptada etc.)."""
    ...
    for (ano, area), bloco in dados.groupby(["NU_ANO", "SG_AREA"]):
        ...
        if chave_validada is not None:
            ...
            for prova in chave_validada:
                if prova in provas_presentes:
...
# ------------------------------------------------------------------
# Filtragem — sempre a aplicação Regular (a única mantida nos dados)
# ------------------------------------------------------------------
filtrado = dados[
